...
def run_attack(checkpoint, x_adv, epsilon):
  ...
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      acc.append(total_corr / num_batches)

  accuracy = total_corr / num_eval_examples

  # acc.append(total_corr)

  print('Accuracy: {:.2f}%'.format(100.0 * accuracy))
  y_pred = np.concatenate(y_pred, axis=0)
  np.save('pred_fgsm2.npy', y_pred)
  print('Output saved at pred_fgsm2.npy')

  # acc = np.concatenate(acc)
  np.save('acc_fgsm2.npy', acc)
  print('Accuracy saved at acc_fgsm2.npy')
...
if __name__ == '__main__':
  ...
  x_adv = np.load(config['store_adv_path2'])
