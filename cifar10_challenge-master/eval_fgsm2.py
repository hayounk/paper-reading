...
from fgsm_attack import LinfFGSMAttack
...
def evaluate_checkpoint(filename):
  with tf.Session() as sess:
    ...
    for ibatch in range(num_batches):
      ...
      print("natural : ", (total_corr_nat, cur_corr_nat))
      print("adv : ", (total_corr_adv, cur_corr_adv))
      loss_a.append(total_xent_adv / num_batches)
      loss_n.append(total_xent_nat / num_batches) 
      
      learning.append(cur_xent_adv / num_eval_examples)
    ...
    # loss_a = np.concatenate(loss_a, axis = 0)
    np.save('loss_fgsm2.npy', loss_a)
    print('Output saved at loss_fgsm2.npy')

    # loss_n = np.concatenate(loss_n, axis = 0)
    np.save('loss_natural2.npy', loss_n)
    print('Output saved at loss_natural2.npy')
